...
func (w *Workloader) runOrderStatus(ctx context.Context, thread int) error {
	...
	if d.cID == 0 {
		...
		query = fmt.Sprintf(query, d.wID, d.dID, d.cLast)
...
WHERE c_id = %d AND c_d_id = %d AND c_w_id = %d`
		...
		query = fmt.Sprintf(query, d.cID, d.dID, d.wID)
		if err := tx.QueryRowContext(ctx, query).Scan(&d.cBalance, &d.cFirst, &d.cMiddle, &d.cLast); err != nil {
			return fmt.Errorf("Exec %s failed %v", query, err)
		}
	}
	...
	query := `SELECT o_id, o_carrier_id, o_entry_d FROM orders WHERE o_w_id = %d
AND o_d_id = %d AND o_c_id = %d ORDER BY o_id DESC LIMIT 1`
	
	query = fmt.Sprintf(query, d.wID, d.dID, d.cID)
	if err := tx.QueryRowContext(ctx, query).Scan(&d.oID, &d.oCarrierID, &d.oEntryD); err != nil {
		return fmt.Errorf("Exec %s failed %v", query, err)
	}
...
FROM order_line WHERE ol_w_id = %d AND ol_d_id = %d  AND ol_o_id = %d`
	...
	query = fmt.Sprintf(query, d.wID, d.dID, d.oID)
	rows, err := tx.QueryContext(ctx, query)
	if err != nil {
		return fmt.Errorf("Exec %s failed %v", query, err)
	}
