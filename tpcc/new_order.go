...
const (
	...
	newOrderSelectCustomerNoPrep = `SELECT c_discount, c_last, c_credit, w_tax FROM customer, warehouse WHERE w_id = %d AND c_w_id = w_id AND c_d_id = %d AND c_id = %d`

	newOrderSelectDistrict       = `SELECT d_next_o_id, d_tax FROM district WHERE d_id = ? AND d_w_id = ? FOR UPDATE`
	newOrderSelectDistrictNoPrep = `SELECT d_next_o_id, d_tax FROM district WHERE d_id = %d AND d_w_id = %d FOR UPDATE`

	newOrderUpdateDistrict       = `UPDATE district SET d_next_o_id = ? + 1 WHERE d_id = ? AND d_w_id = ?`
	newOrderUpdateDistrictNoPrep = `UPDATE district SET d_next_o_id = %d + 1 WHERE d_id = %d AND d_w_id = %d`

	newOrderInsertOrder       = `INSERT INTO orders (o_id, o_d_id, o_w_id, o_c_id, o_entry_d, o_ol_cnt, o_all_local) VALUES (?, ?, ?, ?, ?, ?, ?)`
	newOrderInsertOrderNoPrep = `INSERT INTO orders (o_id, o_d_id, o_w_id, o_c_id, o_entry_d, o_ol_cnt, o_all_local) VALUES (%d, %d, %d, %d, "%s", %d, %d)`

	newOrderInsertNewOrder       = `INSERT INTO new_order (no_o_id, no_d_id, no_w_id) VALUES (?, ?, ?)`
	newOrderInsertNewOrderNoPrep = `INSERT INTO new_order (no_o_id, no_d_id, no_w_id) VALUES (%d, %d, %d)`
	...
	newOrderUpdateStockNoPrep = `UPDATE stock SET s_quantity = %d, s_ytd = s_ytd + %d, s_order_cnt = s_order_cnt + 1, s_remote_cnt = s_remote_cnt + %d WHERE s_i_id = %d AND s_w_id = %d`
)
...
func genNewOrderSelectStockSQL(wID int, items []orderItem) string {
...
func (w *Workloader) runNewOrder(ctx context.Context, thread int) error {
	...
	// Process 1
	if err := s.Conn.QueryRowContext(ctx, fmt.Sprintf(newOrderSelectCustomerNoPrep, d.wID, d.dID, d.cID)).Scan(&d.cDiscount, &d.cLast, &d.cCredit, &d.wTax); err != nil {
	...
	// Process 2
	if err := s.Conn.QueryRowContext(ctx, fmt.Sprintf(newOrderSelectDistrictNoPrep, d.dID, d.wID)).Scan(&d.dNextOID, &d.dTax); err != nil {
	...
	// Process 3
	if _, err := s.Conn.ExecContext(ctx, fmt.Sprintf(newOrderUpdateDistrictNoPrep, d.dNextOID, d.dID, d.wID)); err != nil {
	...
	// Process 4
	if _, err := s.Conn.ExecContext(ctx, fmt.Sprintf(newOrderInsertOrderNoPrep, oID, d.dID, d.wID, d.cID,
	...
	// Process 5
	if _, err := s.Conn.ExecContext(ctx, fmt.Sprintf(newOrderInsertNewOrderNoPrep, oID, d.dID, d.wID)); err != nil {
	...
	for i := 0; i < d.oOlCnt; i++ {
		...
		if _, err = s.Conn.ExecContext(ctx, fmt.Sprintf(newOrderUpdateStockNoPrep, item.sQuantity, item.olQuantity, item.remoteWarehouse, item.olIID, d.wID)); err != nil {
