...
func (w *Workloader) checkCondition1(ctx context.Context, warehouse int) error {
	...
	// 	W_YTD = sum(D_YTD)
	var diff float64
	query := "SELECT sum(d_ytd) - max(w_ytd) diff FROM district, warehouse WHERE d_w_id = w_id AND w_id = %d group by d_w_id"

	query = fmt.Sprintf(query, warehouse)
...
func (w *Workloader) checkCondition2(ctx context.Context, warehouse int) error {
	...
	var diff float64
	query := "SELECT POWER((d_next_o_id -1 - mo), 2) + POWER((d_next_o_id -1 - mno), 2) diff FROM district dis, (SELECT o_d_id,max(o_id) mo FROM orders WHERE o_w_id= %d GROUP BY o_d_id) q, (select no_d_id,max(no_o_id) mno from new_order where no_w_id= %d group by no_d_id) no where d_w_id = %d and q.o_d_id=dis.d_id and no.no_d_id=dis.d_id"

	query = fmt.Sprintf(query, warehouse, warehouse, warehouse)
	rows, err := s.Conn.QueryContext(ctx, query)
	if err != nil {
		return fmt.Errorf("Exec %s failed %v", query, err)
	}
...
func (w *Workloader) checkCondition3(ctx context.Context, warehouse int) error {
	...
	query := "SELECT max(no_o_id)-min(no_o_id)+1 - count(*) diff from new_order where no_w_id = %d group by no_d_id"
...
func (w *Workloader) checkCondition4(ctx context.Context, warehouse int) error {
	...
	query := "SELECT count(*) FROM (SELECT o_d_id, SUM(o_ol_cnt) sm1, MAX(cn) as cn FROM orders,(SELECT ol_d_id, COUNT(*) cn FROM order_line WHERE ol_w_id = %d GROUP BY ol_d_id) ol WHERE o_w_id = %d AND ol_d_id=o_d_id GROUP BY o_d_id) t1 WHERE sm1<>cn"

	query = fmt.Sprintf(query, warehouse, warehouse)
	rows, err := s.Conn.QueryContext(ctx, query)
	if err != nil {
		return fmt.Errorf("Exec %s failed %v", query, err)
	}
...
func (w *Workloader) checkCondition5(ctx context.Context, warehouse int) error {
	...
	query := "SELECT count(*)  FROM orders LEFT JOIN new_order ON (no_w_id=o_w_id AND o_d_id=no_d_id AND o_id=no_o_id) where o_w_id = %d and ((o_carrier_id IS NULL and no_o_id IS  NULL) OR (o_carrier_id IS NOT NULL and no_o_id IS NOT NULL  )) "
...
func (w *Workloader) checkCondition7(ctx context.Context, warehouse int) error {
	...
	query := "SELECT count(*) FROM orders, order_line WHERE o_id=ol_o_id AND o_d_id=ol_d_id AND ol_w_id=o_w_id AND o_w_id = %d AND ((ol_delivery_d IS NULL and o_carrier_id IS NOT NULL) or (o_carrier_id IS NULL and ol_delivery_d IS NOT NULL ))"
...
func (w *Workloader) checkCondition8(ctx context.Context, warehouse int) error {
	...
	query := "SELECT count(*) cn FROM (SELECT w_id,w_ytd,SUM(h_amount) sm FROM history,warehouse WHERE h_w_id=w_id and w_id = %d GROUP BY w_id) t1 WHERE w_ytd<>sm"
...
func (w *Workloader) checkCondition9(ctx context.Context, warehouse int) error {
	...
	query := "SELECT COUNT(*) FROM (select d_id,d_w_id,sum(d_ytd) s1 from district group by d_id,d_w_id) d,(select h_d_id,h_w_id,sum(h_amount) s2 from history WHERE  h_w_id = %d group by h_d_id, h_w_id) h WHERE h_d_id=d_id AND d_w_id=h_w_id and d_w_id= %d and s1<>s2"
...
WHERE c_w_id = %d AND order_count - 2100 != new_order_count`
...
func (w *Workloader) checkCondition12(ctx context.Context, warehouse int) error {
	...
	query := `SELECT count(*) FROM (SELECT  c.c_id, c.c_d_id, c.c_balance c1, c_ytd_payment, 
		...
		O_W_ID=c.c_w_id AND O_D_ID=c.C_D_ID AND O_C_ID=c.C_ID) sm FROM customer c WHERE  c.c_w_id = %d) t1 
